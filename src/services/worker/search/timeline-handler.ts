/**
 * timeline-handler — Extracted from SearchManager.timeline()
 *
 * Anchor-based timeline with observation/session/prompt interleaving.
 * Supports both direct anchor (observation ID, session ID, timestamp)
 * and query-based mode (semantic search to find anchor).
 */
...
export async function timelineHandler(deps: TimelineDeps, args: any): Promise<any> {
