/**
 * Extracted from SearchManager.search()
 * Multi-type search with chroma ranking, date filtering, and file grouping.
 */
...
export interface MultiSearchDeps {
  ...
  sessionSearch: {
    searchObservations: (query: string | undefined, opts: any) => ObservationSearchResult[];
    searchSessions: (query: string | undefined, opts: any) => SessionSummarySearchResult[];
    searchUserPrompts: (query: string | undefined, opts: any) => UserPromptSearchResult[];
  };
  sessionStore: {
    ...
    getUserPromptsByIds: (ids: number[], opts: any) => UserPromptSearchResult[];
  };
  formatter: {
    ...
    formatSessionSearchRow: (sess: SessionSummarySearchResult, lastTime: string) => { row: string; time: string };
    formatUserPromptSearchRow: (prompt: UserPromptSearchResult, lastTime: string) => { row: string; time: string };
  };
}

export async function executeMultiSearch(args: any, deps: MultiSearchDeps): Promise<any> {
